...
namespace HubSpot.Sync.API.Controllers
{
    ...
    {
        private readonly IDealHubspotService dealHubspotService;
        private readonly ILogger<DealController> logger;
        public DealController(IDealHubspotService dealHubspotService, ILogger<DealController> logger)
        {
            this.dealHubspotService = dealHubspotService;
            this.logger = logger;
        }
        ...
        public async Task<IActionResult> Post([FromBody]VerifyModal modal)
        {
            try
            {
                ...
                {
                    await this.dealHubspotService.CreateDeals();
                    return Ok("success");
                }
                else
                {
